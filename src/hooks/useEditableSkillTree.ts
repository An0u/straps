...
function applyLocalStorage(source: Skill[]): Skill[] {
  let result = [...source].map(skill => ({ ...skill, connections: [...skill.connections] }));
...
export const useEditableSkillTree = (sourceSkills: Skill[] = originalData, _gridSize: number = 30) => {
  const [skills, setSkills] = useState<Skill[]>(() => applyLocalStorage(sourceSkills));

  // Re-apply when sourceSkills updates (e.g. after Google Sheets fetch resolves)
  useEffect(() => {
    setSkills(applyLocalStorage(sourceSkills));
  }, [sourceSkills]);
